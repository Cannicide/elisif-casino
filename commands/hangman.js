...
function hangmanStart(prefix, message) {
    if (ls.get(message.guild.id + "hangmanGame")) {
        return message.channel.send("You have already started a game of hangman! Type `" + prefix + "hm end` to end the game early.");
    }
    else {
        var hObj = {
            started: false,
        ...
        message.channel.send("You have begun a game of hangman! Use `" + prefix + "hm guess` to begin guessing!\n\n" + underscores);
    }
}
...
function hangmanEnd(message) {
    if (ls.exist(message.guild.id + "hangmanGame")) {
        if (ls.get(message.guild.id + "hangmanGame").participants[0] == message.author.id || message.member.hasPermission("ADMINISTRATOR")) {
            ...
            message.channel.send(`Ended hangman game, ${message.author.username}!`);
        }
        else {
            message.channel.send(`${message.author.username}, you do not have sufficient permission to end the current hangman game. Only the guild member who started the game and/or the admins of the guild can end a hangman game.`);
        }
    }
    else {
        message.channel.send(`${message.author.username}, you do not have any running games to end.`);
    }
}
...
function hangmanGuess(args, prefix, message) {
    ...
    if (ls.exist(message.guild.id + "hangmanGame")) {
        ...
        if (Number(limit) <= 1) {
            ls.remove(message.guild.id + "hangmanGame");
            return message.channel.send(`Sorry ${message.author.username}, you are out of guesses. **Game over!**\n` + "The word was: **" + word.split("").join(" ") + "**");
        }
        else {
        ...
        if (gotLetter) {
            if (underscores.indexOf("⬜") < 0) {
                ...
                ls.remove(message.guild.id + "hangmanGame");
                message.channel.send("You won hangman! **+2500 dollars to all participants!**");
            }
        }
        else {
            ...
            hObj.guesses = limit;
            ls.set(message.guild.id + "hangmanGame", hObj)
            message.channel.send(`Incorrect guess, ${message.author.username}... ` + limit + " guesses left.");
        }
        }
    }
    else {
        message.channel.send("You must start a game before guessing the word! Use `" + prefix + "hm start` to get started!");
    }
}
...
function hangmanDo(args, message) {
    ...
    if (!action) {
        message.channel.send(`${message.author.username}, please specify whether to start a hangman game, end it, or guess in it.`);
    }
    ...
    if (action.toLowerCase() == "start") {
        ...
        if (subinput) {
            ...
            message.channel.send(`${message.author.username}, please specify a valid letter to guess.`);
        }
    }
        ...
        message.channel.send(`Please specify a valid statement (start|guess|end) to the command. See the Hangman section of \`${prefix}casino\` for more information.`);
    }
}
